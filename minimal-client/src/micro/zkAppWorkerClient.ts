...
import { type ZkAppWorker as ZkAppWorkerType } from '@nori-zk/mina-token-bridge/micro/workers/defs';

export function getZkAppWorker() {
    const worker = new Worker(
        new URL(`./micro.zkAppWorker.${process.env.BUILD_HASH}.js`, import.meta.url),
        {
            type: 'module',
        }
    );
