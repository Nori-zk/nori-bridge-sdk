...
try {
  ...
  const credentialAttestationReady = credentialAttestationWorker.compile();
  ...
  console.log("senderPublicKey.toBase58()", minaSenderPublicKeyBase58);
  ...
  const contract = new ethers.Contract(noriETHBridgeAddressHex, abi, ethWallet);
  ...
  depositProcessingStatusSubscription = depositProcessingStatus$.subscribe({
    next: console.log,
    error: console.error,
    complete: () =>
      console.warn(
  ...
  //await tokenMintWorker.WALLET_setMinaPrivateKey(minaSenderPrivateKeyBase58);
  ...
  const { ethDepositProofJson } = await tokenMintWorker.computeEthDeposit(
    presentationJsonStr,
    depositBlockNumber,
    ethAddressLowerHex
  );
  console.log("Computed eth deposit proof.", ethDepositProofJson);

  console.log("IF WE GOT THIS FAR IN THE FRONT END WE PROBABLY KOOL");
