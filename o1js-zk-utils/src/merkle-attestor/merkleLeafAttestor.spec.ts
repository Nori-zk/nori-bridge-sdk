...
describe('Merkle Attestor Test', () => {
    test('compute_non_provable_storage_slot_leaf_hash', () => {
        ...
        console.log(
            `Hash result bytes: ${wordToBytesCanonical(hash, 32).map((byte) =>
                byte.toNumber()
            )}`
        );
        console.log(
            `Hash result hex: ${wordToBytesCanonical(hash, 32)
        ...
        console.log(
            `Provable hash result hex: ${wordToBytesCanonical(hash2, 32)
                .map((byte) => byte.toNumber().toString(16).padStart(2, '0'))
                .join('')}`
        );
    });
