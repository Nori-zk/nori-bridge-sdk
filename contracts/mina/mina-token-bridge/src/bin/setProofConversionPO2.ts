...
// Other imports
import { Mina, PrivateKey, Field, type NetworkId, fetchAccount } from 'o1js';
...
if (issues.length) {
    const formatted = [
        'SetProofConversionPO2 encountered issues:',
...
async function setProofConversionPO2() {
    ...
    const txn = await Mina.transaction(
        ...
        async () => {
            logger.log(
                `Setting proofConversionPO2 to: '${possiblePO2Value}'`
            );
