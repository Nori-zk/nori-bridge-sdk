/**
 * NoriTokenBridge Worker-driven Full E2E Test Suite (Livenet)
 *
 * Mirrors NoriTokenBridge.full.lightnet.integration.spec.ts — happy path,
 * negative tests, and the 40-root window rotation block — but runs against
 * a real Mina network (mesa devnet via getStagingEnv() defaults) instead of
 * lightnet's /acquire-account endpoint.
 *
 * Pre-funded accounts (deployer, admin, alice, dave) are sourced from
 * LOAD_USER_MINA_PRIV_KEYS (same env var used by loadRunner.ts). Requires
 * at least 4 comma-separated base58 Mina private keys; each must hold
 * enough MINA to cover fees + new-account funding for the full suite.
 *
...
new LogPrinter('TestMinaNoriTokenBridgeWorkerFullLivenet');
const logger = new Logger('WorkerFullIntegrationLivenetTest');


const fee = Number(process.env.MINA_TX_FEE ?? 0.05) * 1e9;
...
/**
 * Pre-funded livenet accounts. Same env var loadRunner.ts uses
 * (LOAD_USER_MINA_PRIV_KEYS): comma-separated base58 Mina private keys.
 * The first 4 are consumed as deployer / admin / alice / dave; each must
 * hold enough MINA to cover fees + new-account funding for the full suite.
 */
function getLivenetFundedKeypairs(minCount: number): Keypair[] {
    const raw = process.env.LOAD_USER_MINA_PRIV_KEYS;
    if (!raw) {
        throw new Error(
            'LOAD_USER_MINA_PRIV_KEYS is required for livenet integration test (comma-separated base58 Mina private keys; needs >= 4 funded accounts)'
        );
    }
    const keys = raw.split(',').map((s) => s.trim()).filter(Boolean);
    if (keys.length < minCount) {
        throw new Error(
            `LOAD_USER_MINA_PRIV_KEYS requires at least ${minCount} keys (deployer, admin, alice, dave); got ${keys.length}`
        );
    }
    return keys.slice(0, minCount).map((sk) => {
        let privateKey: PrivateKey;
        try {
            privateKey = PrivateKey.fromBase58(sk);
        } catch (err) {
            throw new Error(
                `LOAD_USER_MINA_PRIV_KEYS contains an invalid base58 private key: ${String(err)}`
            );
        }
...
describe('NoriTokenBridge (Worker-driven, full)', () => {
    beforeAll(async () => {
        ...
        networkOptions = {
            ...
            mina:
                ...
                staging.MINA_RPC_NETWORK_URL,
            archive:
                process.env.MINA_ARCHIVE_RPC_URL ??
                staging.MINA_ARCHIVE_RPC_URL,
        };

        const Network = Mina.Network(networkOptions);
        Mina.setActiveInstance(Network);

        // dave (4th key) is consumed inside the noriMint describe's beforeAll.
        const fundedKeys = getLivenetFundedKeypairs(4);
        deployer = fundedKeys[0];
        admin = fundedKeys[1];
        alice = fundedKeys[2];
      ...
      tokenBasePriv   ${tokenBaseKeypair.privateKey.toBase58()}
      noriTokenBridge ${noriTokenBridgeKeypair.publicKey.toBase58()}
      noriTokenBridgePriv ${noriTokenBridgeKeypair.privateKey.toBase58()}
    `);
    ...
    describe('noriMint()', () => {
        ...
        beforeAll(async () => {
            ...
            // 4th funded key from LOAD_USER_MINA_PRIV_KEYS — same source
            // used in the outer beforeAll for deployer/admin/alice.
            dave = getLivenetFundedKeypairs(4)[3];
