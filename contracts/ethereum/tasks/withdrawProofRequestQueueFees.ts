...
const logger = new Logger('WithdrawProofRequestQueueFees');

export const withdrawProofRequestQueueFees = task('withdrawProofRequestQueueFees', 'Withdraw accumulated proof request queue fees from the proof request queue')
    .setAction(async () => ({
        default: async (_args, hre) => {
            ...
            if (issues.length) {
                logger.error('WithdrawProofRequestQueueFees encountered errors:');
                issues.forEach((issue, idx) => logger.warn(`  ${idx + 1}: ${issue}`));
                logger.fatal('Due to issues with environment variables withdrawProofRequestQueueFees cannot continue.');
                process.exit(1);
            }
            ...
            const feeRecipient = await proofQueue.feeRecipient();
            const accumulatedFees = await proofQueue.accumulatedFees();
            ...
            let tx;
            try {
                tx = await proofQueue.withdrawFees();
            } catch (err: unknown) {
                const data = err instanceof Object && 'data' in err ? (err as { data: string }).data : null;
                const reason = data ? proofQueue.interface.parseError(data) : null;
                if (reason) {
            ...
            const feesEvent = receipt.logs
                .map((log: { topics: string[]; data: string }) => { try { return proofQueue.interface.parseLog(log); } catch { return null; } })
