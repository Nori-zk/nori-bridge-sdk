...
The finding is correct and the recommendation is accepted. The deposit set is no longer supplied by the prover at all. It is replaced by an on-chain, append-only **proof request queue** on Ethereum, which the guest is forced to drain in order.

A new contract, `NoriProofRequestQueue`, holds a monotonic `head` counter and a mapping of numbered request records. Every `lockTokens` call appends one request. Each request records the `target` contract whose storage is to be proven (always `msg.sender`, so a contract can only request proofs of its own state), the storage `slotKey`, and up to two `collectionKeys` that ride along into the proven leaf.

The Mina bridge stores a `queueCursor` — how many requests have been settled. An `update` must resume exactly at that cursor, and on success advances it. Batch start, batch size and each entry's storage location are therefore all **derived** rather than chosen:

- start is the cursor the Mina contract already holds,
- `head` is read from the queue's storage by Merkle-Patricia proof against the proven execution state root,
- size is `min(head - cursor, MAX_BATCH)`, and a witness whose entry count disagrees is rejected,
- each entry's five storage words are located from its index by Solidity's mapping and struct-member rules, not taken from the witness.

The prover has no discretion at any of these points, so an empty or pruned root can no longer be produced. The all-zero root remains reachable only when `head == cursor`, which is itself proven — at genesis this is an exclusion proof that the head slot is absent from the trie.

Completeness is total rather than best-effort: every entry in range contributes a leaf. A populated slot is pinned by an inclusion proof of its true value; an empty slot, or a target account that does not exist, by an exclusion proof of zero. "Holds zero" and "absent from the trie" are the same fact in a Merkle-Patricia trie, and for a given root and key exactly one of the two proofs can verify, so the claim is pinned either way. A junk request therefore proves as zero and the queue rolls on — it can neither stall the queue nor be silently skipped. A proof that verifies as neither aborts the run: a committable failure outcome would reintroduce the omission this finding describes.

This also addresses the footgun raised in the recommendation. Completeness is enforced **incrementally**, not all-at-once: a batch is the outstanding backlog, capped at `MAX_BATCH` (2^16, the Merkle tree's leaf capacity), and anything beyond that cap drains over consecutive updates, in order, with no entry skipped.

Forcing a large batch is bounded by what enqueueing costs. The queue drains every update window, so a backlog only builds within one window, and each `requestProof` writes three to five cold storage words and pays `proofRequestQueueFee` on top — through `lockTokens`, a whole deposit of at least `MIN_LOCK_AMOUNT_WEI` as well, with the fees accruing to the protocol. Reaching the cap inside a window would take several times the blockspace that window has, so the cap bounds the tree rather than describing a state an attacker can force. A larger batch costs proving time and fees; it cannot leave the queue stuck.

On the preemption scenario specifically: a frontrunner's proof must resume from the same cursor and carry the same entries, so it can no longer advance the head while omitting deposits. On the data-availability point the audit places out of scope, the queue is append-only and never deleted, so any committed batch's leaf set is reconstructible from Ethereum state by any party.
...
- **Documentation**: the doc comment on the removed `genesisRoot` field, which described it as an immutable chain anchor preventing store-hash rotation from redirecting the bridge, is deleted with the field. `contracts/DEPLOYMENT.md` and the Ethereum README drop the genesis-root inputs, the pre-deploy record and the ledger row, and carry the proof request queue address in their place. The admin trust assumptions set out in the response above are recorded in this entry, which is the reference for them until they are carried into the deployment documentation.
