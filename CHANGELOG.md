...
## 23/9/26 - Remove Aligned Layer from contracts/ethereum

Aligned Layer is deprecated and the `npm run build` script's `git clone` of `aligned_layer` was failing in CI on one of its nested submodules.

- `contracts/ethereum/contracts/MinaAccountValidation.sol`: deleted. Wrapped `AlignedLayerServiceManager.verifyBatchInclusion` for Mina zkApp account proofs.
- `contracts/ethereum/contracts/MinaStateSettlement.sol`: deleted. Wrapped Aligned Layer batch inclusion verification for Mina ledger state.
- `contracts/ethereum/contracts/NoriTokenBridge.sol`: removed the `stateSettlement` and `accountValidation` state variables, `setAlignedContracts`, `isConfigured`, `onlyConfigured`, and the `AlignedContractsNotConfigured` error. `unlockTokens()` now takes no arguments and always reverts with `InvalidZkappAccount`. `lockTokens` no longer has the `onlyConfigured` modifier. The constructor keeps `_zkappAcctTokenId` and `_zkappAcctVerificationKeyHash`, since they bind unlock verification to the correct Mina token namespace regardless of which proof system checks it; it no longer takes `_stateSettlementAddr` or `_accountValidationAddr`.
- `contracts/ethereum/bin/preDeploy.ts`: deleted. Fetched `ALIGNED_ETH_SERVICE_MANAGER_ADDRESS` from the `aligned_layer` GitHub repo.
- `contracts/ethereum/.env.nori-eth-pre-deploy.example`: deleted.
- `contracts/ethereum/.env.nori-eth-token-bridge.example`: removed the `ALIGNED_ETH_SERVICE_MANAGER_ADDRESS` line, its header comment, and the `(pre-deploy)` note.
- `contracts/ethereum/.gitignore`: removed the `lib` entry.
- `contracts/ethereum/foundry.lock`: deleted. Pinned the `aligned_layer` submodule commit for `forge`, which this project does not invoke.
- `contracts/ethereum/ignition/modules/NoriTokenBridge.ts`: deleted. Not invoked by any script; its constructor call had been stale since before this change.
- `contracts/ethereum/test.mocha`: deleted. Not referenced anywhere; `hardhat.config.ts` defines its own inline mocha config.
- `contracts/ethereum/package.json`: `build` no longer clones `aligned_layer`. Added `@openzeppelin/contracts` as a direct dependency, pinned to `5.0.2`, the exact version `aligned_layer` vendored (its `ReentrancyGuard._status` is a plain slot 0 variable at this version, matching the storage layout `lockedTokens` depends on). Removed the `pre-deploy` script. `lint` and `lint:fix` no longer reference the deleted `bin` directory.
- `contracts/ethereum/remappings.txt`: now a single line, `@openzeppelin/contracts/=node_modules/@openzeppelin/contracts/`. Removed the `aligned_layer`, `eigenlayer-middleware`, `eigenlayer-core`, `forge-std`, `openzeppelin-contracts-upgradeable`, `risc0-ethereum`, `sp1-contracts`, and `eigenlayer-contracts` remappings.
- `contracts/ethereum/tasks/deploy.ts`: no longer deploys `MinaAccountValidation` or `MinaStateSettlement`, and no longer reads or validates `ALIGNED_ETH_SERVICE_MANAGER_ADDRESS`. `NoriTokenBridge.deploy(...)` is called with `proofQueue.target` in place of the two deleted contracts' addresses. The written `.env.nori-eth-token-bridge` no longer includes `NORI_ETH_MINA_STATE_SETTLEMENT_ADDRESS` or `NORI_ETH_MINA_ACCOUNT_VALIDATION_ADDRESS`.
- `contracts/ethereum/test/NoriTokenBridge.ts`: removed the `setAlignedContracts` and `isConfigured` describe blocks and every `dummyState`/`dummyAccount` fixture signer and constructor argument. The admin gating assertion in "Should allow deploying with a different bridgeOperator than deployer" now calls `setLockFeeRate` instead of the deleted `setAlignedContracts`.
- `contracts/ethereum/README.md`: removed the Aligned layer env var block, the Pre-deploy section, and the two deleted contracts from the Deploy section's description and sample output.
- `contracts/DEPLOYMENT.md`: removed the AlignedLayer specific wording from the mainnet blocker note, section 3 and section 6, marking the tokenId and verification key hash checks as blocked on a verifier being wired into `unlockTokens` rather than describing a procedure that no longer applies. Removed the pre-deploy section entirely and renumbered every section after it. Section 8 (deploy) now describes the two contracts actually deployed, with the current 5 parameter constructor. Removed `MinaStateSettlementAddress` and `MinaAccountValidationAddress` from the final ledger. Appendix A's constructor signature and Appendix B's fixture-test item are updated to match.
- `contracts/ethereum/artifacts/**` and `contracts/ethereum/types/ethers-contracts/**`: regenerated by `npm run build`.
- `package-lock.json`: updated for the `@openzeppelin/contracts` addition.
